    ...
    internal class SJFPreemptive
    {
        public static (float, float, List<GanttBlock>) SJFP(List<Process> processes)
        {
            ...
            for (int i = 0; i < arrTime.Count; i++)
            {
                ...
                sortedProcesses = sortedProcesses.OrderBy(p => p.burstTime).ThenBy(p => p.arrivalTime).ToList();
                ...
                ganttBlocks.Add(new GanttBlock(pMaxPriority.name, arrTime[i], arrTime[i + 1]));
            }
